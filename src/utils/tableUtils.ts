...
import type { PageSizeOptions, Pagination, TableData, TableProps, TableState } from '@/types/common';
...
export function tableUtils() {
  ...
  const pageSizeOptions = ref<PageSizeOptions[]>([
    { title: 10, value: 10 },
    { title: 15, value: 15 },
    { title: 25, value: 25 },
    { title: 50, value: 50 },
  ...
  function checkUncheckAll(event: Event, props: TableProps) {
    if (props.tableConfig) {
      const checked = (event.target as HTMLInputElement).checked;

      props.tableConfig.data.forEach((item: TableData) => {
        setSelectedItem(checked, Number(item.id));
      });
    }
  }

  function onItemChecked(event: Event) {
    setSelectedItem((<HTMLInputElement>event.target)?.checked, Number((<HTMLInputElement>event.target)?.value));
  }

  function setSelectedItem(checked: Boolean, value: Number) {
    const index = tableState.selected.indexOf(Number(value));
    if (checked) {
      if (index == -1) tableState.selected.push(Number(value));
    } else {
      tableState.selected = tableState.selected.filter(id => id !== Number(value));
    }
  }

  function onSort(field: string, props: TableProps) {
  ...
  function handleDate(selectedDates: Date[], dateStr: string) {
  ...
  function getColSpan(props: TableProps) {
  ...
  function applyFilters(props: TableProps) {
    let filteredData;
    ...
    if (props.tableConfig) {
      ...
      if (tableState.filter['search'].trim() !== '') {
        filteredData = filteredData.filter((item: TableData) => {
          return Object.keys(item).some(key => {
            const value = item[key as keyof TableData] as SearchableValue;
      ...
      if (tableState.filter['sort']) {
        filteredData.sort((a: TableData, b: TableData): number => {
          const key = tableState.sortableKey as keyof TableData;
          const valueA = a[key];
          const valueB = b[key];
      ...
      if (tableState.filter['date']['start_date']) {
        filteredData = filteredData.filter((data: TableData) => {
